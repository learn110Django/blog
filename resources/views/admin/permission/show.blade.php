...
<div class="card">
  ...
  <a href="{{route('permission.create')}}" class="float-right btn btn-primary btn-md">Add  Role</a>
  </div>
  <div class="card-body">
    ...
    <table id="example1" class="table table-bordered table-striped">
      ...
      <tbody>
          ...
          <td>{{$permission->name}}</td>
            ...
            <td> <a href="{{route('permission.edit',$permission->id)}}" class="btn btn-primary btn-sm"><i class="fas fa-edit"></i></a></td>
            <td> 

              <form action="{{route('permission.destroy',$permission->id)}}" method="POST" style="display: none" id="delete-id-{{$permission->id}}">
                  @csrf
                  @method('DELETE')
              </form>
             <a href="" 
             ...
             document.getElementById('delete-id-{{$permission->id}}').submit();}
              else{
               event.preventDefault();
              }
             " 
